...
class ViserScene:
    def __init__(self, robotGeometryModel, viserServer, mesheFilesPaths={}):
        ...
        self.h_jaxes   = {}
        self.h_jsliders= {}
        self.q = {joint.name:0.0 for joint in self.robot.joints.values()}
        self.visual_attrs = {
            "axes_radius" : 0.005,
            "axes_length" : 0.25,
...
#                    fullScenePath, mesh=mesh)
    ...
    def _addToScene(self, link, parent, scenePathToParent):
        ...
        if parent is None : # the base link of the robot; the scene parent is thus the root element
            ...
            self.scene.add_icosphere(scenePath, radius=0.002,
    ...
    def loadAndDisplay(self):
        ...
        with self.gui.add_folder(label="Frames & Co.", expand_by_default=False):
            opts = ["show","hide"]
            group = self.gui.add_tab_group()
            def addTabWithFramesCtrls(label, rob_items, frames_h):
                with group.add_tab(label):
                    h_buttons = self.gui.add_button_group(label="All:", options=opts)
                    checkboxes = []
                    ...
                    for name in rob_items:
                        h = self.gui.add_checkbox(label=name, initial_value=frames_h[name].visible)
            ...
            addTabWithFramesCtrls("Joint axes", [n for n,j in self.robot.joints.items() if j.kind!=JointKind.fixed], self.h_jaxes)
            addTabWithFramesCtrls("Others", self.robotGeometry.framesModel.userFrames, self. h_uframes)


        with self.gui.add_folder(label="Meshes"):
            h = self.gui.add_slider(label="opacity", min=0.0, max=1.0, initial_value=1.0, step=0.1)
            h.on_update(func= lambda ev:
                self._setOpacity(self.h_meshes.values(), ev.target.value))

        with self.gui.add_folder(label="Explode"):
            h = self.gui.add_slider(label="distance", min=0, max=1, initial_value=0, step=0.05)
            h.on_update(func= lambda ev:
                self._setExplodeDistance(self.h_links.values(), ev.target.value))

        with self.gui.add_folder(label="Joint state", expand_by_default=False):
            for jname in self.robot.joints:
                axis = self.robotGeometry.jointAxes.get(jname)
                if axis:
                    h = self.gui.add_slider(label=jname, min=-np.pi, max=np.pi, initial_value=self.q[jname], step=0.1)
                    h.on_update(func= lambda ev: self._setJointState( self.robot.joints[ev.target.label], ev.target.value ) )
                    self.h_jsliders[jname] = h
    ...
    def _setExplodeDistance(self, handles, dist):
        for h in handles :
            ...
            if joint:
                axis = self.robotGeometry.jointAxes.get(joint.name)
                if axis:
                    tr = self.jointTransforms[joint.name]
                    H = tr.eval( self.q[joint.name] ) if len(tr.variables) > 0 else tr.eval()
                    pos = H[0:3,3]
                    pos = pos + np.array(axis)*dist
                    h.position = pos

    def _setJointState(self, joint, value):
        self.q[joint.name] = value
        H = self.jointTransforms[joint.name].eval( value )
        se3 = viser.transforms.SE3.from_matrix(matrix=H)
        h_link = self.h_links[ self.robot.successor(joint).name ]
        h_link.position = se3.translation()
        h_link.wxyz     = se3.rotation().wxyz
