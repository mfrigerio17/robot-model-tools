...
def collapseFixedJoints(connectivity, ordering=None, frames=None, geometry=None, inertia=None):
    ...
    for fixj in fixedJoints:
        ...
        allDroppedLinks.add(link1)
        allDroppedLinks.add(link2)
    ...
    jointToLinkPair = { jo : connectivity.jointToLinkPair(jo)
        for jo in connectivity.joints.values() if jo.kind != JointKind.fixed }
    newpairs = [ KPair( joint = joint,
                        link1 = toNewLink[pair[0]],
                        link2 = toNewLink[pair[1]] ) for joint,pair in jointToLinkPair.items() ]
    ...
    if inertia is not None:
        ...
        inertialData = { name:inertia.byLink(link) for name,link in connectivity.links.items() if link not in allDroppedLinks }
